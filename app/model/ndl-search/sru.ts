...
export function composeSearchUrl(params: SruSearchParameters) {
	...
	if (params.startRecord)
		url.searchParams.append("startRecord", params.startRecord.toString());
	if (params.maximumRecords)
		url.searchParams.append("maximumRecords", params.maximumRecords.toString());
...
export async function search(params: SruSearchParameters) {
	...
	const dcNdlDocument =
		json.searchRetrieveResponse.records[0].record.recordData;
