...
export function serializeCql(cql: CqlQuery): string {
	...
	if (cql.query.creator)
		parts.push(serializeAnyOrAll("creator", cql.query.creator));
	if (cql.query.publisher)
		parts.push(serializeAnyOrAll("publisher", cql.query.publisher));
	if (cql.query.digitizedPublisher)
		parts.push(
			serializeAnyOrAll("digitizedPublisher", cql.query.digitizedPublisher),
		);
	if (cql.query.ndc) parts.push(`ndc=${cql.query.ndc}`);
	if (cql.query.ndlc) parts.push(`ndlc=${cql.query.ndlc}`);
	if (cql.query.description)
		parts.push(serializeAnyOrAll("description", cql.query.description));
	if (cql.query.subject)
		parts.push(serializeAnyOrAll("subject", cql.query.subject));
	...
	if (cql.query.anywhere)
		parts.push(serializeAnyOrAll("anywhere", cql.query.anywhere));
	if (cql.query.itemno) parts.push(`itemno=${cql.query.itemno}`);
	if (cql.query.mediatype)
		parts.push(serializeAny("mediatype", cql.query.mediatype));
