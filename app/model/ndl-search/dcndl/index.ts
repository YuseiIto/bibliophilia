...
export class DcNdlParser {
	...
	get sourceIdentifier() {
		return (this.bibResource["dcndl:sourceIdentifier"] ?? []).map(
			mapRdfDatatype,
		);
	}
	...
	get alternativeVolume() {
		return (this.bibResource["dcndl:alternativeVolume"] ?? []).map(
			mapRdfDescription,
		);
	}

	get alternativeVolumeTitle() {
		return (this.bibResource["dcndl:alternativeVolumeTitle"] ?? []).map(
			mapRdfDescription,
		);
	}
	...
	get publicationPlace() {
		return (this.bibResource["dcndl:publicationPlace"] ?? []).map(
			mapRdfDatatype,
		);
	}
	...
	get dctermsSubject() {
		return (this.bibResource["dcterms:subject"] ?? []).map(
			(x: RdfDescription | RdfResource) => {
				return isRdfDescription(x) ? mapRdfDescription(x) : mapRdfResource(x);
			},
		);
	}
	...
	get originalLanguage() {
		return (this.bibResource["dcndl:originalLanguage"] ?? []).map(
			mapRdfDatatype,
		);
	}
