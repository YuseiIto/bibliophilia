...
describe("エルマーのぼうけん(test-fixtures/my-fathers-dragon.rdf)", async () => {
	...
	test("dcterms:identifier", () => {
		const expected = [
			...
			{
				datatype: "http://ndl.go.jp/dcndl/terms/NDLBibID",
				value: "000000814611",
			},
		];
	...
	test("dcterms:creator", () => {
		const expected = [
			{
				about: "http://id.ndl.go.jp/auth/entity/00440494",
				name: "Gannett, Ruth Stiles, 1923-",
			},
			{
				about: "http://id.ndl.go.jp/auth/entity/00618242",
				name: "Gannett, Ruth Chrisman, 1896-1979",
			},
			{
				about: "http://id.ndl.go.jp/auth/entity/00091023",
				name: "渡辺, 茂男, 1928-2006",
				transcription: "ワタナベ, シゲオ, 1928-2006",
			},
		];
	...
	test("dcterms:issued", () => {
		expect(parser.dctermsIssued).toMatchObject([
			{ datatype: "http://purl.org/dc/terms/W3CDTF", value: "1963" },
		]);
	});
...
describe("プログラミングTypeScript (test-fixtures/programming-typescript.rdf)", async () => {
	...
	test("dcterms:identifier", () => {
		const expected = [
			...
			{
				datatype: "http://ndl.go.jp/dcndl/terms/TOHANMARCNO",
				value: "34046612",
			},
			{
				datatype: "http://ndl.go.jp/dcndl/terms/ISBN",
				value: "978-4-87311-904-5",
			},
		];
		expect(parser.dctermsIdentifier).toMatchObject(expected);
	});
	...
	test("dcterms:title", () => {
		expect(parser.dctermsTitle).toBe(
		...
		expect(parser.dcTitle).toMatchObject([
			{
				value:
					"プログラミングTypeScript : スケールするJavaScriptアプリケーション開発",
				transcription:
					"プログラミング タイプ スクリプト : スケール スル ジャヴァ スクリプト アプリケーション カイハツ",
			},
		]);
	});
	...
	test("dcterms:creator", () => {
		const expected = [
			{
				about: "http://id.ndl.go.jp/auth/entity/001344802",
				name: "Cherny, Boris",
			},
			{
				about: "http://id.ndl.go.jp/auth/entity/001344803",
				name: "今村, 謙士",
			},
			{
				about: "http://id.ndl.go.jp/auth/entity/00552631",
				name: "原, 隆文, 1965-",
				transcription: "ハラ, タカフミ, 1965-",
			},
		];
	...
	test("dcterms:subject", () => {
		expect(parser.dctermsSubject).toMatchObject([
			{
				transcription: "プログラミング (コンピュータ)",
				value: "プログラミング (コンピュータ)",
			},
